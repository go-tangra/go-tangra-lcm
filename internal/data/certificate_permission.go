...
// ListAll lists all permissions with optional filters (admin view, no tenant restriction)
func (r *CertificatePermissionRepo) ListAll(
	ctx context.Context,
	certificateID *string,
	granteeClientID *string,
	page, pageSize *uint32,
	...
	entities, err := query.
		...
		All(ctx)
	if err != nil {
		r.log.Errorf("list all certificate permissions failed: %s", err.Error())
		return nil, 0, lcmV1.ErrorInternalServerError("list certificate permissions failed")
	}
