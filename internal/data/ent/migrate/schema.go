...
var (
	...
	IssuedCertificatesColumns = []*schema.Column{
		...
		{Name: "is_external", Type: field.TypeBool, Comment: "True if adopted from an external CA (not originally issued by LCM)", Default: false},
		{Name: "acme_directory_url_override", Type: field.TypeString, Nullable: true, Comment: "Per-cert ACME directory URL (e.g. DigiCert order-specific renew URL). Overrides the issuer endpoint when set."},
	...
	IssuedCertificatesTable = &schema.Table{
		...
		ForeignKeys: []*schema.ForeignKey{
			{
				...
				Columns:    []*schema.Column{IssuedCertificatesColumns[31]},
				RefColumns: []*schema.Column{LcmClientsColumns[0]},
				OnDelete:   schema.SetNull,
			},
		},
		Indexes: []*schema.Index{
			...
			{
				Name:    "issuedcertificate_is_external",
				Unique:  false,
				Columns: []*schema.Column{IssuedCertificatesColumns[12]},
			},
			{
				Name:    "issuedcertificate_expires_at",
				Unique:  false,
				Columns: []*schema.Column{IssuedCertificatesColumns[26]},
			},
			{
