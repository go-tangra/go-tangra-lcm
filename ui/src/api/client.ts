...
/**
 * Calls the lcm API through the gateway. Non-2xx responses reject with ApiError
 * carrying the server's closed-vocabulary `reason`.
 */
...
export function describe(err: unknown): string {
  ...
  switch (err.reason) {
    ...
    case 'relation_above_granter':
      return 'You cannot grant a relation above your own.'
    case 'not_found':
      return 'Not found.'
    case 'conflict':
      return 'A sibling with that name already exists, or the change is not possible.'
