...
// 1. mTLS: extracts CN, maps to client_id, looks up tenant
// 2. Admin-service proxy: reads tenant ID directly from metadata (set by middleware)
func (s *IssuerService) getClientTenantID(ctx context.Context) (uint32, error) {
	clientInfo, ok := client.GetClientInfoFromContext(ctx)
	if !ok || clientInfo == nil || !clientInfo.IsAuthenticated {
	...
	// mTLS path: resolve CN to client_id and look up tenant
	certCN := clientInfo.CommonName
	if certCN == "" {
		return 0, lcmV1.ErrorUnauthorized("client authentication required")
	}
