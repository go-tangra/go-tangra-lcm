...
func (tx *Tx) init() {
	tx.AcmeIssuer = NewAcmeIssuerClient(tx.config)
	tx.AuditLog = NewAuditLogClient(tx.config)
	tx.CertificateDetails = NewCertificateDetailsClient(tx.config)
	...
	tx.LcmCa = NewLcmCaClient(tx.config)
	tx.LcmClient = NewLcmClientClient(tx.config)
	tx.MtlsCertificate = NewMtlsCertificateClient(tx.config)
	tx.MtlsCertificateRequest = NewMtlsCertificateRequestClient(tx.config)
	tx.SelfSignedIssuer = NewSelfSignedIssuerClient(tx.config)
	tx.TenantSecret = NewTenantSecretClient(tx.config)
}
...
// applies a query, for example: AcmeIssuer.QueryXXX(), the query will be executed
