...
// TestRegister checks the request lcm sends to auth (feature 019): module,
...
func TestRegister(t *testing.T) {
	...
	req := auth.got
	if auth.method != authv1.Authorization_RegisterPermissions_FullMethodName || req.GetModule() != "lcm" || req.GetModuleDisplayName() != "Certificates" || !req.GetDeclaresRoles() {
		t.Fatalf("%s %v", auth.method, req)
	}
	if len(req.GetPermissions()) != len(lcmmanifest.Permissions) {
	...
	if len(roles) != 3 || len(roles["administrator"]) != len(lcmmanifest.Permissions) || len(roles["operator"]) != 8 || len(roles["viewer"]) != 3 {
		t.Fatalf("%v", roles)
	}
	...
	for slug, refs := range lcmmanifest.Grants {
