...
// ListAllPermissions lists all permissions across all certificates (admin view)
func (s *CertificatePermissionService) ListAllPermissions(ctx context.Context, req *lcmV1.ListAllPermissionsRequest) (*lcmV1.ListAllPermissionsResponse, error) {
	permissions, total, err := s.permissionRepo.ListAll(ctx, req.CertificateId, req.GranteeClientId, req.Page, req.PageSize)
	if err != nil {
		return nil, err
	}
	...
	return &lcmV1.ListAllPermissionsResponse{
		Permissions: protoPermissions,
		Total:       uint32(total),
	}, nil
}
