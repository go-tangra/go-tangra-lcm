...
func (IssuedCertificate) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Bool("is_external").
			Default(false).
			Comment("True if adopted from an external CA (not originally issued by LCM)"),

		field.String("acme_directory_url_override").
			Optional().
			Comment("Per-cert ACME directory URL (e.g. DigiCert order-specific renew URL). Overrides the issuer endpoint when set."),
